...
export function db(): ReturnType<typeof postgres> {
  if (!sql) {
    ...
    if (!url) throw new Error('Missing APP_DATABASE_URL or GBRAIN_DATABASE_URL');
...
export async function getGbrainAuth(): Promise<GbrainAuth | null> {
  ...
  return {
    oauth_client_id: rows[0].oauth_client_id as string,
    oauth_client_secret: rows[0].oauth_client_secret as string,
  };
}
...
export async function getUserByApiKey(apiKey: string): Promise<AppUser | null> {
  ...
  `;
  if (rows.length === 0) return null;
  return { id: rows[0].id as string, api_key: rows[0].api_key as string };
}
...
export async function listRecentMessages(sessionId: string, limit = 12): Promise<AppMessage[]> {
  const rows = await db()`
...
export async function insertMemory(userId: string, slug: string, content: string): Promise<AppMemory> {
  const rows = await db()`
