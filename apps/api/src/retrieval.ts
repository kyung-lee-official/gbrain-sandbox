...
export function logRetrievalHits(
  ...
  console.log(
    `[query hits] ${label} q=${JSON.stringify(query)} n=${hits.length}`,
  );
  if (hits.length === 0) {
    console.log("  (no hits)");
    return;
  ...
  for (const [i, hit] of hits.entries()) {
    const ratio = topScore > 0 ? (hit.score / topScore).toFixed(3) : "n/a";
...
function numericFactorsFromRow(
...
function readRequiredFloatEnv(name: string): number {
  ...
  if (!raw) {
    throw new Error(
      `${name} is required for ask-mode page hydrate. Set it in .env at the repo root.`,
    );
  }
