 ...
 * App schema: Prisma. Users: `bun run seed`.
 *
...
import { closeDb, getGbrainAuth, upsertGbrainAuth } from "../src/db.ts";
...
async function main(): Promise<void> {
  ...
  if (!appDatabaseUrl()) {
    throw new Error(
      "Set APP_DATABASE_URL (app DB, e.g. gbrain_app). Run Prisma migrate before this script.",
    );
  }
  ...
  console.log("\nSetup complete.");
  console.log("Seed users:  bun run seed");
